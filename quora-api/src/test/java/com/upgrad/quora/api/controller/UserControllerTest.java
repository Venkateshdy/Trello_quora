...
public class UserControllerTest {
    ...
    public void signupWithRepeatedUserName() throws Exception {
        mvc.perform (
                MockMvcRequestBuilders.post (
                        "/user/signup?firstName=a&lastName=a&userName=database_username&emailAddress=a&password=a&country=a&aboutMe=a&dob=a&contactNumber=a")
    ...
    public void signupWithRepeatedEmail() throws Exception {
        mvc.perform (
                MockMvcRequestBuilders.post (
                        "/user/signup?firstName=a&lastName=a&userName=non_existing_username&emailAddress=database_email&password=a&country=a&aboutMe=a&dob=a&contactNumber=a")
                        .contentType (MediaType.APPLICATION_JSON_UTF8))
                .andExpect (status ().isConflict ())
                .andExpect (MockMvcResultMatchers.jsonPath ("code").value ("SGR-002"));
    }

    // This test case passes when you try to signout but the JWT token entered does not exist in the
    // database.
    @Test
    public void signoutWithNonExistingAccessToken() throws Exception {
        mvc.perform (
                MockMvcRequestBuilders.post ("/user/signout")
